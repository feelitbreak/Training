...
namespace BitOperationsTask
{
    ...
    public static class NumbersExtension
    {
        ...
        public static int InsertNumberIntoAnother(int destinationNumber, int sourceNumber, int i, int j)
        {
            ...
            {
                if (sourceNumber.BitIsZero(pos - i))
                {
                    destinationNumber = destinationNumber.MakeBitZero(pos);
                }
                else
                {
                    destinationNumber = destinationNumber.MakeBitOne(pos);
                }
            }
        ...
        private static bool BitIsZero(this int number, int pos)
        {
            int mask = 1 << pos;
            return (number & mask) == 0;
        }

        private static int MakeBitZero(this ref int number, int pos)
        {
            int mask = 1 << pos;
            return number & ~mask;
        }

        private static int MakeBitOne(this int number, int pos)
        {
            int mask = 1 << pos;
            return number | mask;
        }
